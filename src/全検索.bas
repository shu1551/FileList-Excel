...
' ※このプロシージャが単独で呼ばれる想定の場合
    ...
    ' 3. 実際の最終セル（ゴースト含む）を取得
    '    フォルダ一覧等でA列/B列の埋まりが揃わず取りこぼすのを防ぐ
    On Error Resume Next
    cellLastRow = ActiveSheet.Cells.SpecialCells(xlCellTypeLastCell).Row
    On Error GoTo 0
    
    ' 4. 3つのうち最大を削除対象の最終行とする
    lastRow = textLastRow
    If usedLastRow > lastRow Then lastRow = usedLastRow
    If cellLastRow > lastRow Then lastRow = cellLastRow
...
End Sub
Sub マクロの一覧()
Attribute マクロの一覧.VB_ProcData.VB_Invoke_Func = "M\n14"
Application.ScreenUpdating = False

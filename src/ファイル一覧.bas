...
Sub 選択しているエクセルのマクロ削除()
    ...
    With wBookB
        With .VBProject
            ...
            ' VBEコンポーネントをループ処理
            ' （削除しながら回すため逆順インデックス。For Each だと削除時に次の要素がスキップされる）
            For k = wComponents.Count To 1 Step -1
                Set wCompo = wComponents(k)
...
Sub ファイル削除()
    ...
    ' --- 安全対策3：削除完了の待機と確認 ---
    ' ごみ箱への移動は非同期なので、消えるまで最大5秒待つ（通常は一瞬で抜ける）
    Dim t0 As Single
    t0 = Timer
    Do While FSO.FileExists(fullPath)
        DoEvents ' OSに制御を戻す
        If Timer < t0 Then t0 = t0 - 86400 ' 日付またぎ補正
...
Set FSO = CreateObject("Scripting.FileSystemObject")
    ...
    If Sheets("設定").Range("A8").Value <> "" Then
       ...
       If Right(copysaki, 1) <> "\" Then copysaki = copysaki & "\"
    Else
    ...
    ' 移動元の存在確認
    If Not FSO.FileExists(motoname) Then
        MsgBox "移動元ファイルが見つかりません。" & vbCrLf & motoname, vbExclamation
        Set FSO = Nothing
        Application.ScreenUpdating = True
        Exit Sub
    End If
...
newSaveFilePath = create_new_file_path(Copyfilename)
    ...
    If Err.Number <> 0 Then
        MsgBox "コピーに失敗したため移動を中止しました。" & vbCrLf & _
               Err.Description & vbCrLf & newSaveFilePath, vbCritical
        On Error GoTo 0
        Set FSO = Nothing
        Application.ScreenUpdating = True
        Exit Sub
    ...
    ' 元ファイル削除（失敗したらコピーを取り消して移動中止＝二重化を防ぐ）
    Err.Clear
    Kill motoname
    If Err.Number <> 0 Then
        FSO.DeleteFile newSaveFilePath
        On Error GoTo 0
        MsgBox "元ファイルを削除できないため移動を中止しました。" & vbCrLf & _
               "（開いているか、ロックされています）" & vbCrLf & motoname, vbExclamation
        Set FSO = Nothing
        Application.ScreenUpdating = True
        Exit Sub
    End If
...
Sub フォルダ抽出の実行()
    ...
    Call sagyouフォルダ(path01, SKIP_SUB)
...
Sub sagyouフォルダ(ByVal startPath As String, ByVal skipSub As Boolean)
